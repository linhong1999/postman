...
urlpatterns = [
    ...
    re_path(r'v1/pyq_operate/(?P<pyq_user_name>.*)/$',
            views.PyqOperator.as_view({'delete': 'del_restore_pyq', 'put': 'update_pyq','post': 'update_wink_status'})),

    re_path(r'v1/comment_operate/(?P<pyq_obj_id>.*)/(?P<comment_obj_id>.*)$',
            views.PyqOperator.as_view({'post': 'add_comment', 'delete': 'del_comment'})),


    path('v1/test/', views.PyqOperator.as_view({'post': 'test'})),
    re_path(r'v1/test/(?P<pyq_obj_id>.*)/$', views.PyqBrowser.as_view({'post': 'test'})),
